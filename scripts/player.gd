...
export var initialx = 16
export var initialy = 176
...
func time_loop() -> void:
	...
	if rewind:
		GLOBAL.player_rewind()
		#GLOBAL.fast_restart_scene()
...
func shoot() -> void:
	...
	if last_direction < 0:
		bullet.get_node("area/sprite").flip_h = false
		push = -40 - push
	else:
		bullet.get_node("area/sprite").flip_h = true
	bullet.position.x += push
	bullet.velocity.x = last_direction
